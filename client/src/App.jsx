...
function CaseDetail({ caseId, onBack, onUpdated, user, onProfile }) {
  ...
  return (
    <div className="workspacePage detailPage">
      ...
      <AssignmentPanel caseId={caseId} item={item} onChanged={async()=>{await load();onUpdated();}} /><CaseCalendarPanel caseId={caseId}/><CaseTasksPanel caseId={caseId}/>
...
function BillingView(){
  ...
  return <div className="workspacePage"><div className="workspaceHeading"><div><span>TARIF & ABRECHNUNG</span><h1>{org?'MängelFix Verwaltung':'MängelFix Privat'}</h1><p>{org?'Testphase, Tarifstatus und Nutzung deines Verwaltungs-Arbeitsbereichs.':'Tarifstatus deines persönlichen Kontos.'}</p></div></div>{error&&<div className="errorBox">{error}</div>}<section className="workspacePanel billingHero"><div><span>AKTUELLER STATUS</span><h2>{trial?`14-Tage-Testphase · noch ${days} Tag${days===1?'':'e'}`:p.subscription_status==='active'?'Aktiv':'Nicht aktiv'}</h2><p>{org?'Während der Testphase stehen die Verwaltungsfunktionen innerhalb der unten aufgeführten Limits zur Verfügung.':'Dein Privatkonto bleibt unabhängig von Verwaltungs-Arbeitsbereichen.'}</p></div><div className="billingActions">{data.checkoutConfigured&&data.cycles?.monthly&&<button className="primaryButton" disabled={busy} onClick={()=>checkout('monthly')}>Monatlich wählen</button>}{data.checkoutConfigured&&data.cycles?.yearly&&<button className="primaryButton" disabled={busy} onClick={()=>checkout('yearly')}>Jährlich wählen</button>}{!data.checkoutConfigured&&<button className="primaryButton" disabled>Online-Zahlung noch nicht aktiviert</button>}{p.subscription_customer_id&&<button className="secondaryButton" disabled={busy} onClick={portal}>Abo verwalten</button>}</div></section>{org&&<div className="billingUsage"><article><span>TEAMMITGLIEDER</span><b>{data.usage.members} / {p.max_members}</b></article><article><span>OBJEKTE</span><b>{data.usage.properties} / {p.max_properties}</b></article><article><span>EINHEITEN</span><b>{data.usage.units} / {p.max_units}</b></article></div>}<section className="workspacePanel billingInfo"><h2>Abrechnung vorbereitet</h2><p>Tarifstatus, Testende, Zahlungsanbieter, Customer-/Subscription-ID und Abrechnungszeitraum sind bereits technisch vorgesehen. Konkrete Europreise und Stripe Checkout werden erst aktiviert, wenn die Preise endgültig festgelegt sind.</p></section></div>;
}
...
function Workspace({ user, setUser, onLogout, navigate }) {
  ...
  useEffect(() => { loadCases(); refreshUnread(); api('/api/management/overview').then(setManagement).catch(()=>setManagement({organization:null})); api('/api/admin/me').then(x=>setIsAdmin(Boolean(x.admin))).catch(()=>setIsAdmin(false)); const params=new URLSearchParams(window.location.search); const caseId=params.get('case'); if(caseId)setSelected(caseId); }, []);
  const profileComplete = Boolean(user.street && user.postalCode && user.city);
  ...
  return <div className="workspaceShell"><aside className="workspaceSidebar"><button className="sidebarBrand" onClick={() => setView('overview')}><Logo inverse /></button><div className="sidebarLabel">ARBEITSBEREICH</div><nav><button className={view === 'overview' && !selected ? 'active' : ''} onClick={() => { setSelected(null); setView('overview'); }}><span>Ü</span>Übersicht</button><button className={view === 'analytics' ? 'active' : ''} onClick={() => { setSelected(null); setView('analytics'); }}><span>Q</span>{management?.organization?'Analyse':'Auswertung'}</button><button className={view === 'cases' || selected ? 'active' : ''} onClick={() => { setSelected(null); setView('cases'); }}><span>M</span>Mängel <b>{cases.filter(x => x.status !== 'resolved').length}</b></button><button className={view === 'search' ? 'active' : ''} onClick={() => { setSelected(null); setView('search'); }}><span>S</span>Suche & Archiv</button>{!management?.organization&&<button className={view === 'connections' ? 'active' : ''} onClick={() => { setSelected(null); setView('connections'); }}><span>V</span>Verknüpfte Verwaltungen</button>}<button className={view === 'objects' ? 'active' : ''} onClick={() => { setSelected(null); setView('objects'); }}><span>O</span>Objekte</button><button className={view === 'deadlines' ? 'active' : ''} onClick={() => { setSelected(null); setView('deadlines'); }}><span>F</span>Fristen <b>{cases.filter(x => x.deadline_on && x.status !== 'resolved').length}</b></button><button className={view === 'documents' ? 'active' : ''} onClick={() => { setSelected(null); setView('documents'); }}><span>D</span>Dokumente</button>{management?.organization&&<><button className={view === 'providers' ? 'active' : ''} onClick={() => { setSelected(null); setView('providers'); }}><span>H</span>Dienstleister</button><button className={view === 'orders' ? 'active' : ''} onClick={() => { setSelected(null); setView('orders'); }}><span>A</span>Aufträge</button></>}<button className={view === 'inspections' ? 'active' : ''} onClick={() => { setSelected(null); setView('inspections'); }}><span>Ü</span>Übergabe / Abnahme</button><button className={view === 'calendar' ? 'active' : ''} onClick={() => { setSelected(null); setView('calendar'); }}><span>K</span>Kalender</button><button className={view === 'tasks' ? 'active' : ''} onClick={() => { setSelected(null); setView('tasks'); }}><span>✓</span>Aufgaben</button><button className={view === 'notifications' ? 'active' : ''} onClick={() => { setSelected(null); setView('notifications'); }}><span>B</span>Benachrichtigungen {unreadNotifications>0&&<b>{unreadNotifications}</b>}</button>{management?.organization&&<button className={view === 'audit' ? 'active' : ''} onClick={() => { setSelected(null); setView('audit'); }}><span>A</span>Aktivitätsprotokoll</button>}<button className={view === 'billing' ? 'active' : ''} onClick={() => { setSelected(null); setView('billing'); }}><span>€</span>Tarif & Abrechnung</button><button className={view === 'team' ? 'active' : ''} onClick={() => { setSelected(null); setView('team'); }}><span>T</span>{management?.organization ? 'Team' : 'Verwaltung'}</button>{isAdmin&&<button className={view === 'admin' ? 'active' : ''} onClick={() => { setSelected(null); setView('admin'); }}><span>K</span>Kamilunavo Admin</button>}</nav><div className="sidebarBottom"><button className={view === 'profile' ? 'active' : ''} onClick={goProfile}><span>P</span>Profil {!profileComplete && <i />}</button><button onClick={() => navigate('/')}><span>↗</span>Startseite</button><div className="sidebarUser"><div>{user.name.slice(0, 1).toUpperCase()}</div><p><b>{user.name}</b><span>{user.email}</span></p><button onClick={onLogout} title="Abmelden">↪</button></div></div></aside><main className="workspaceMain"><div className="mobileWorkspaceBar"><Logo compact /><button onClick={() => setShowNew(true)}>+ Neuer Mangel</button></div>{error && <div className="workspaceGlobalError">{error}</div>}{content}</main>{showNew && <NewCase onClose={() => setShowNew(false)} onCreated={created => { setShowNew(false); loadCases(); setSelected(created.id); }} />}</div>;
}
