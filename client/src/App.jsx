    ...
    <div className="landingPage">
      ...
      <main>
        ...
        <section className="featureSection" id="funktionen">
          <div className="featureHeading"><span>DEIN DIGITALER MÄNGELORDNER</span><h2>Ein Werkzeug für den kompletten Mängelverlauf.</h2></div>
          <div className="featureGrid">
            <article className="featureLarge"><div className="featureNumber">01</div><h3>Alle Fälle im Blick</h3><p>Offen, versendet, in Bearbeitung oder erledigt. Du siehst sofort, wo etwas noch Aufmerksamkeit braucht.</p><div className="miniCaseList"><span><i className="dot amber" />Paket beschädigt <b>OFFEN</b></span><span><i className="dot blue" />Werkstatt-Nachbesserung <b>VERSENDET</b></span><span><i className="dot green" />Mietmangel Bad <b>ERLEDIGT</b></span></div></article>
  ...
  const content = {
    ...
    datenschutz: {
      ...
      sections: [
        ...
        ['2. Welche Daten MängelFix verarbeitet', <p>Bei der Nutzung verarbeiten wir insbesondere Kontodaten, freiwillige Profildaten, erfasste Mängel- und Vorgangsdaten, Referenz- und Empfängerdaten, Fristen, Aufgaben, Termine, Nachrichten und Notizen sowie hochgeladene Fotos und Dokumente. Bei Verwaltungsarbeitsbereichen können außerdem Objekt-, Einheiten-, Mieter-, Dienstleister-, Arbeitsauftrags-, Übergabe- und Protokolldaten verarbeitet werden.</p>],
        ['3. Zweck und Rechtsgrundlage', <p>Die Daten werden verarbeitet, um Konten und Arbeitsbereiche bereitzustellen, Mängel zu dokumentieren und nachzuverfolgen, Kommunikation, Fristen, Termine und Arbeitsaufträge zu organisieren, Dokumente zu erzeugen und die Anmeldung abzusichern. Soweit dies für die Bereitstellung des Dienstes erforderlich ist, erfolgt die Verarbeitung zur Durchführung des Nutzungsverhältnisses. Technisch erforderliche Sicherheitsmaßnahmen dienen dem sicheren und stabilen Betrieb.</p>],
        ...
        ['9. Deine Rechte', <p>Nach den anwendbaren Datenschutzvorschriften können dir insbesondere Rechte auf Auskunft, Berichtigung, Löschung, Einschränkung der Verarbeitung, Datenübertragbarkeit und Widerspruch zustehen. Außerdem besteht ein Beschwerderecht bei einer zuständigen Datenschutzaufsichtsbehörde.</p>],
        ['10. Sicherheit', <p>Wir setzen technische und organisatorische Maßnahmen ein, um personenbezogene Daten gegen unbefugten Zugriff, Verlust und Missbrauch zu schützen. Passwörter werden nicht im Klartext gespeichert.</p>],
        ['11. Stand', <p>Stand: 13. August 2026. Diese Datenschutzerklärung wird angepasst, wenn sich Funktionen oder Datenverarbeitungen von MängelFix ändern.</p>]
      ]
    },
    terms: {
      ...
      sections: [
        ...
        ['8. Tarife und Zahlungen', <p>Kostenpflichtige Funktionen, Preise, Abrechnungszeiträume und Kündigungsmöglichkeiten werden vor dem verbindlichen Abschluss eines Abonnements angezeigt. Solange die Zahlungsfunktion nicht aktiviert ist, entsteht durch die bloße Nutzung einer Test- oder kostenlosen Version kein kostenpflichtiges Abonnement.</p>],['9. Stand', <p>Stand: 13. August 2026.</p>]
      ]
    }
  }[type];
...
function NewCase({ onClose, onCreated, pro=true, entitlements=null }) {
  ...
  const [form, setForm] = useState({ title:'',caseContext:'housing',category:categories[0],description:'',propertyLabel:'',locationLabel:'',subjectLabel:'',referenceLabel:'',counterpartyType:'',purchaseOn:'',purchasePrice:'',warrantyUntil:'',desiredResolution:'',discoveredOn:today,recipientName:'',recipientEmail:'',recipientAddress:'',deadlineOn:'',destinationLinkId:'' });
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState('');
  ...
  return (
    <div className="modalBackdrop" onMouseDown={onClose}>
      <div className="modal proModal" onMouseDown={e => e.stopPropagation()}>
        ...
        <form onSubmit={submit} className="caseForm">{!pro&&<div className="freeCaseHint"><b>Privat Free</b><span>{entitlements?.usage?.activeCases||0} / 5 aktive Vorgänge · Fristen und erweiterte Organisation mit Pro.</span></div>}
          <label>Worum geht es bei diesem Mangel?<select value={form.caseContext} onChange={e=>{const next=e.target.value;setForm(current=>({...current,caseContext:next,category:categoriesForContext(next)[0]}));}}>{Object.entries(caseContexts).map(([k,v])=><option key={k} value={k}>{v.label}</option>)}</select></label><div className="formGrid two"><label>Titel<input required placeholder={form.caseContext==='housing'?'z. B. Heizung bleibt kalt':'z. B. Lieferung beschädigt angekommen'} value={form.title} onChange={e => field('title', e.target.value)} /></label><label>Kategorie<select value={form.category} onChange={e => field('category', e.target.value)}>{categoriesForContext(form.caseContext).map(item => <option key={item}>{item}</option>)}</select></label></div>
          ...
          <div className="subSection"><h3>{form.destinationLinkId?'Zusätzlicher Empfänger (optional)':caseContexts[form.caseContext].recipient}</h3><p className="muted">Optional – die Stelle, an die du den Mangel melden oder dokumentieren möchtest.</p></div>
...
function ProFeatureCard({title,text,onUpgrade}){return <section className="contentCard proFeatureCard"><div><span>PRIVAT PRO</span><h3>{title}</h3><p>{text}</p></div><button className="primaryButton" onClick={onUpgrade}>Pro ansehen · 4,99 €</button></section>}
...
function CaseDetail({ caseId, onBack, onUpdated, user, onProfile, pro=true, onUpgrade }) {
  ...
  return (
    <div className="workspacePage detailPage">
      ...
      <div className="detailGrid">
        <section className="contentCard"><div className="cardKicker">FALLINHALT</div><h3>Beschreibung</h3><p className="descriptionText">{item.description}</p><div className="factsGrid"><div><span>{data.viewerRole==='management'?'Objekt':(caseContexts[item.case_context||'housing']?.subject||'Bezug')}</span><b>{item.subject_label||item.property_label||'—'}</b></div><div><span>{data.viewerRole==='management'?'Raum / Ort':(caseContexts[item.case_context||'housing']?.place||'Ort / Bereich')}</span><b>{item.location_label||'—'}</b></div><div><span>Rückmeldung bis</span><b>{fmtDate(item.deadline_on)}</b></div><div><span>Empfänger</span><b>{item.recipient_name || 'Noch nicht hinterlegt'}</b></div></div></section>
      ...
      <section className="contentCard"><div className="sectionTitle"><div><div className="cardKicker">BEWEISSICHERUNG</div><h3>Fotos, Dokumente & Belege</h3><p className="muted">{pro||data.viewerRole==='management'?'Vorher/Nachher-Fotos, Rechnungen, Lieferscheine oder PDF-Dokumente direkt am Vorgang.':'Privat Free: bis zu 3 normale Fotos. Dokumente und erweiterte Belegtypen gibt es mit Pro.'}</p></div>{pro||data.viewerRole==='management'?<label className="secondaryButton uploadButton">Belege hinzufügen<input type="file" accept="image/*,application/pdf" multiple onChange={async e=>{const fd=new FormData();[...e.target.files].forEach(f=>fd.append('files',f));try{await api(`/api/cases/${caseId}/evidence`,{method:'POST',body:fd});await load();onUpdated()}catch(err){setError(err.message)}e.target.value=''}} /></label>:<label className="secondaryButton uploadButton">Fotos hinzufügen · max. 3<input type="file" accept="image/*" multiple onChange={uploadImages}/></label>}</div>{data.attachments.length ? <div className="photoGrid">{data.attachments.map(file => <a key={file.id} href={`/api/attachments/${file.id}`} target="_blank" rel="noreferrer">{file.mime_type==='application/pdf'?<div className="pdfEvidence">PDF</div>:<img src={`/api/attachments/${file.id}`} alt={file.original_name} />}<span>{file.original_name}{file.evidence_type&&file.evidence_type!=='photo'?` · ${file.evidence_type}`:''}</span></a>)}</div> : <div className="emptyInline">Noch keine Bilder hinterlegt.</div>}</section>
...
function BillingView(){
 ...
 if(!data)return <div className="workspacePage"><div className="emptyCard">Tarif wird geladen…</div></div>;const org=data.scope==='organization',p=data.plan,trial=p.subscription_status==='trialing',days=p.trial_ends_at?Math.max(0,Math.ceil((new Date(p.trial_ends_at)-new Date())/86400000)):null,plans=data.catalog||[];const price=x=>cycle==='yearly'?x.yearly:x.monthly;const planNames={private_free:'Privat Free',private_pro:'Privat Pro',management_starter:'Verwaltung Starter',management_pro:'Verwaltung Pro',management_business:'Verwaltung Business'};const currentPlanName=planNames[p.plan_code]||'Privat Free';
 return <div className="workspacePage billingPage"><div className="workspaceHeading"><div><span>TARIF & ABRECHNUNG</span><h1>{org?'MängelFix Verwaltung':'MängelFix Privat'}</h1><p>{org?'Tarif nach verwalteten Einheiten wählen.':'Free dauerhaft nutzen oder mit Privat Pro alle Werkzeuge freischalten.'}</p></div><div className="billingCycle"><button className={cycle==='monthly'?'active':''} onClick={()=>setCycle('monthly')}>Monatlich</button><button className={cycle==='yearly'?'active':''} onClick={()=>setCycle('yearly')}>Jährlich · günstiger</button></div></div>{error&&<div className="errorBox">{error}</div>}<section className="workspacePanel billingHero"><div className="billingStatusIcon"><Icon name={trial?'deadlines':'billing'}/></div><div className="billingStatusCopy"><span>DEIN AKTUELLER TARIF</span><h2>{trial?`Kostenlose Testphase`:currentPlanName}</h2><p>{trial?`Noch ${days} Tag${days===1?'':'e'} alle Verwaltungsfunktionen testen.`:org?'Dein Tarif richtet sich nach der Anzahl verwalteter Einheiten.':'Die Grundfunktionen bleiben dauerhaft kostenlos. Ein Upgrade ist jederzeit möglich.'}</p></div><div className="billingStatusMeta"><span>{trial?'TESTPHASE':p.plan_code==='private_free'?'KOSTENLOS':'AKTIV'}</span>{p.subscription_customer_id&&<button className="secondaryButton" disabled={busy} onClick={portal}>Zahlungen & Rechnungen</button>}</div></section>{org&&data.usage&&<div className="billingUsage"><article><span>TEAMMITGLIEDER</span><b>{data.usage.members} / {p.max_members}</b></article><article><span>OBJEKTE</span><b>{data.usage.properties} / {p.max_properties}</b></article><article><span>EINHEITEN</span><b>{data.usage.units} / {p.max_units}</b></article></div>}<div className={`billingPlanGrid ${org?'managementPlans':'privatePlans'}`}>{plans.map(plan=>{const free=price(plan)===0,configured=Boolean(data.checkoutConfigured&&plan.checkout?.[cycle]),current=p.plan_code===plan.code;return <article className={`billingPlanCard ${plan.code.includes('_pro')?'featured':''} ${current?'current':''}`} key={plan.code}><div className="planCardTop"><span>{current?'DEIN TARIF':plan.scope==='private'?'PRIVAT':'VERWALTUNG'}</span>{current&&<b><i/> Aktiv</b>}</div><h2>{plan.name}</h2><div className="billingBigPrice"><b>{price(plan).toLocaleString('de-DE',{minimumFractionDigits:free?0:2})} €</b><small>{free?'dauerhaft kostenlos':cycle==='yearly'?'/ Jahr':'/ Monat'}</small></div>{plan.maxUnits&&<p><b>Bis {plan.maxUnits} Einheiten</b> · {plan.maxMembers} Mitarbeiter</p>}<ul>{plan.code==='private_free'?<><li>Bis 5 aktive Vorgänge</li><li>Bis 3 Fotos je Vorgang</li><li>Basis-PDF & Verlauf</li><li>Manuelle Empfänger</li></>:plan.code==='private_pro'?<><li>Unbegrenzte aktive Vorgänge</li><li>Erweiterte Fotos, PDFs & Belege</li><li>Fristen, Aufgaben & Kalender</li><li>Archiv, Analysen & Übergabeprotokolle</li></>:<><li>Alle Mängelmanagement-Funktionen</li><li>Team, Mieter & Dienstleister</li><li>Kalender, Aufgaben & Auswertungen</li><li>14 Tage kostenlos testen</li></>}</ul>{free?<button className="secondaryButton" disabled>Im Konto enthalten</button>:current?<button className="secondaryButton" disabled>Aktiver Tarif</button>:configured?<button className="primaryButton" disabled={busy} onClick={()=>checkout(plan.code)}>{busy?'Einen Moment…':cycle==='yearly'?'Jahrestarif wählen':'Monatstarif wählen'}</button>:<button className="primaryButton" disabled>Zahlung wird beim Server-Setup aktiviert</button>}</article>})}</div></div>;
}
...
function CaseCalendarPanel({caseId,readOnly=false}){
...
function TeamView({ onWorkspaceChanged }) {
  ...
  if (!team.organization) {
    return <div className="workspacePage teamOnboarding"><div className="workspaceHeading"><div><span>HAUSVERWALTUNG</span><h1>Ein gemeinsamer Arbeitsbereich für dein Team.</h1><p>Privat bleibt privat. Verwaltungsfälle, Objekte und Mitarbeiter bekommen einen getrennten, professionellen Bereich.</p></div><div className="teamTrialBadge"><b>14 TAGE</b><span>kostenlos testen</span></div></div><div className="teamIntroGrid"><section className="workspacePanel teamSetup"><div className="teamSetupIntro"><div className="teamSetupIcon"><Icon name="objects"/></div><div><span>SCHRITT 1 VON 1</span><h2>Hausverwaltung anlegen</h2><p>Du wirst Inhaber und kannst anschließend Objekte sowie Mitarbeiter hinzufügen.</p></div></div><form onSubmit={createOrganization}><label><span>Name der Hausverwaltung</span><div className="teamInputWrap"><Icon name="objects"/><input required placeholder="z. B. Muster Hausverwaltung GmbH" value={orgName} onChange={e => setOrgName(e.target.value)} /></div></label>{error && <div className="errorBox">{error}</div>}{message && <div className="successBox">{message}</div>}<button className="primaryButton" disabled={busy}>{busy ? 'Arbeitsbereich wird erstellt…' : <>Arbeitsbereich kostenlos einrichten <b>→</b></>}</button><small>Keine Zahlungsdaten erforderlich · Der Privattarif bleibt unverändert</small></form></section><aside className="teamBenefits"><div className="teamBenefitsHead"><Icon name="team"/><span>VERWALTUNGSBEREICH</span></div><h3>Direkt bereit für den ersten Vorgang</h3><p>Alles, was dein Team für einen nachvollziehbaren Ablauf braucht.</p><ul><li><i>✓</i><span><b>Eigene Mitarbeiter-Logins</b>Kein gemeinsam genutztes Passwort</span></li><li><i>✓</i><span><b>Klare Rollen</b>Inhaber, Admin und Mitarbeiter</span></li><li><i>✓</i><span><b>Gemeinsame Vorgänge</b>Mängel, Dokumente und Fristen im Team</span></li><li><i>✓</i><span><b>Objekte strukturiert verwalten</b>Gebäude, Einheiten und Kontakte zuordnen</span></li></ul></aside></div></div>;
  }
...
function Workspace({ user, setUser, onLogout, navigate }) {
  ...
  else if (view === 'deadlines') content = <DeadlineCenterView onSelect={setSelected} />;
